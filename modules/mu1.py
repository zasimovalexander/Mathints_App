...
from values import OPS_MAP, DRW_MAP
...
def calcs(
    ...
    def _adt_sub_filling(seed_is: list[list[str | int]]) -> None:
        data_draw[k] = seed_is + [_meta_seq([f"{nr1} {OPS_MAP[k]} {nr2}", str(dr)], "clr2"),
    ...
    for idx, ftn in enumerate(fracts["ftn"]):
        ...
        if factor > 1:  # apply normalization only if it changes the fraction
            ...
            if lcm_drs != nums[2 * idx + 1]:  # check to avoid duplicating the original (pre-simplified) fraction
                for ftn_, clr in zip(([f"{prt}{OPS_MAP["mul"]}{factor}" for prt in ftn], _to_str((nr, lcm_drs))),
                                     ("clr2", "clr0")):
    ...
    operations = [(k, OPS_MAP["larger" if nr1 > nr2 else ("equal", "less")[nr1 < nr2]]), ]
    ...
    k = "sub"
    seed_sub = seed.copy()
    if operations[0][1] == OPS_MAP["less"]:
        seed_sub.reverse()
        nr1, nr2 = nr2, nr1
    nr = nr1 - nr2
...
def _simplify_if(
    ...
    if gcd_ftn > 1:
        ...
        for ftn, clr in zip(([f"{prt}{OPS_MAP["mul"]}{gcd_ftn}" for prt in nd], _to_str(nd)),
                            ("clr1", clr_end)):
...
def _output(
    ...
    for idx in range(2):
        ...
        if add_seq:
            ...
            _draw_text(runner_x - step_hlf, runner_y, DRW_MAP[add_seq[3]])
        runner_x = _rend_hblock(data[k], runner_x, runner_y, position_ys)
        if idx == 0:
            _draw_text(runner_x + step, runner_y, DRW_MAP["clr4"], names[0][1], DRW_MAP["font_ops"])
            runner_x += 2 * step
    ...
    for k, sgn in names[1:]:
        ...
        _draw_text(runner_x + step, runner_y, DRW_MAP["clr4"], sgn, DRW_MAP["font_ops"])
        runner_x += 2 * step
...
def _draw_text(
        ...
        clr: str,
        t: str =OPS_MAP["equal"],
        fnt: tuple[str | int, ...] =DRW_MAP["font"]
) -> None:
